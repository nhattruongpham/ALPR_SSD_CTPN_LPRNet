...
class TextImageGenerator:
    def __init__(self, img_dir, label_file, batch_size, img_size, num_channels=3, label_len=5):
...
def get_train_model(num_channels, label_len, b, img_size):
    ...
    x = conv(x,256,NUM_CHARS+1,ksize=[1,16],pad='SAME') # change ksize [1,13] to [1,11]. change ksize [1,11] to [1,16]
    x = tf.nn.relu(x)
...
def train(a):
    ...
    #                                            staircase=True)
    logits, inputs, seq_len = get_train_model(num_channels, label_len,BATCH_SIZE, img_size)
    ...
    def test_report(testi,files):
        ...
        for i in range(num):
            test_inputs, test_seq_len = val_gen.next_batch()
            test_feed = {inputs: test_inputs,
    ...
    def do_batch(train_gen,val_gen):
        ...
        if steps > 0 and steps % REPORT_STEPS == 0:
            ...
            saver.save(session, "./model/LPRNetVN.ckpt", global_step=steps)
        return b_cost, steps
